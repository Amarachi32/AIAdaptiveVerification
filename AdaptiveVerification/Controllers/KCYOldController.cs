...
namespace AdaptiveVerification.Controllers
{
    ...
    [ApiController]
    public class KCYOldController : ControllerBase
    {
        private readonly IConfiguration _configuration;
        private readonly Kernel _kernel;
        public KCYOldController(IConfiguration configuration)
        {
...
                """;*/
                ...
                string prompt = $@"
                You are a KYC assistant. Based on the user submission info (name, email, account age, and geolocation), please evaluate the risk level and assign a KYC verification method.
                Risk window mapping:
                - 00:00–01:00 → low
                - 02:00–05:00 → medium
                - 06:00–10:00 → high
                - Additional considerations for critical risk may include unusual patterns, flagged behaviors, or submissions from high-risk locations.
                For low: PIN verification.
                For medium: PIN + OTP.
                For high: Facial Recognition.
                For critical: Immediate manual review required.
                Current UTC time: {DateTime.UtcNow}
                Name: {customerData.Name}
                Email: {customerData.Email}
                Please provide a detailed risk assessment that includes:
                1. Risk Level (CRITICAL, HIGH, MEDIUM, LOW)
                2. Recommended KYC Verification Method
                3. Justification for each risk level considered (low, medium, high, critical) based on the provided data.
                Make sure to evaluate all risk levels and provide reasoning for each.
                ";
